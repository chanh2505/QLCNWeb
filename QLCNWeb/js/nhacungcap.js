...
function getAllPartners() {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ IsSupplier: true })
    }).done(function (data) {
        var html = "";
                    ...
                    callback: function () {
                        ...
                        $.ajax({
                            ...
                            if (data == "1") {
                                ...
                                bootbox.alert("Đã xóa nhà cung cấp thành công!");
                            }
                            else if(data == "0")
                            {
                                bootbox.alert("Không thể xóa nhà cung cấp đã có giao dịch!");
                            }
                            else
                            {
    ...
    groupElements.btnSave.on('click', function () {
        ...
        if (groupElements.form.valid()) {
            ...
            $.ajax({
                ...
                if (data && data.ID > 0) {
                    if (currentItem && currentItem.ID > 0) {
                        bootbox.alert("Đã cập nhật nhà cung cấp thành công!!!");
                    }
                    else {
                        bootbox.alert("Đã thêm nhà cung cấp thành công!!!");
                        clear();
                    }
                    getAllPartners().done(function () {
                        closeWaitDialog();
                    ...
                    closeWaitDialog();
                    bootbox.alert("Thêm nhà cung cấp thất bại, liên hệ admin để được hỗ trợ!!!");
                }
            }).fail(function (b) {
                closeWaitDialog();
                bootbox.alert("Thêm nhà cung cấp thất bại, liên hệ admin để được hỗ trợ!!!");
